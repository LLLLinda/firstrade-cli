...
const { exchangeCredential, renewCookies, getCurrentXml, getTimeLeft } = require('./core');
...
class Firstrade {
  ...
  async getBalance(credential) {
    ...
    if (!res.response.balances) return [];
  ...
  async getTradeHistory(credential) {
    ...
    if (!res.response.orderstatus) return [];
  ...
  async getPosition(credential) {
    ...
    if (!res.response.position) return [];
...
module.exports.default = firstrade;
